...
class Graph(object):
    ...
    def get_nodes(self):
        ...
        "*** YOUR CODE HERE ***"
        """
        Returns the nodelist object of the node called
        """
    ...
    def get_inputs(self, node):
        ...
        "*** YOUR CODE HERE ***"
        """
        checks that the nodes parents exist for the base case
        if parents exist creates a list of the output of each parent node
    ...
    def add(self, node):
        ...
        if node not in self.nodes:
            ...
            values0 = node.forward(nodeInputs)
            values1 = np.zeros_like(values0)
            self.nodes[node] = [values0, values1]
    ...
    def backprop(self):
        ...
        count = len(self.get_nodes())
        count -= 2
        #set gradiant of the loss_node
        values = self.nodes[loss_node]
        grad = loss_node.backward(self.get_inputs(loss_node), [1.0])
        self.nodes[loss_node] = [values[0], 1.0]
        newc = 0
        for parent in loss_node.get_parents():
            vals = self.nodes[parent]
            self.nodes[parent] = [vals[0], vals[1] + grad[newc]]
            newc += 1
        while count >= 0:
            node = nodelist[count]
            values = self.nodes[node]
            nodegrad = node.backward(self.get_inputs(node), values[1])
            nodeC = 0
            for parent in node.get_parents():
                parentValues = self.nodes[parent]
                self.nodes[parent] = [parentValues[0], parentValues[1] + nodegrad[nodeC]]
                nodeC += 1
            count -= 1
    ...
    def step(self, step_size):
        ...
        count = 0
        while count < self.numVariables:
            node = nodes[count]
            node.data = node.data - step_size*self.get_gradient(node)
            count += 1
...
class MatrixMultiply(FunctionNode):
    ...
    def backward(inputs, gradient):
        ...
        a = np.transpose(a)
        b = np.transpose(b)
        newgrad = [np.dot(gradient, b), np.dot(a, gradient)]
        return newgrad
...
class MatrixVectorAdd(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        #print "sum grad", np.sum(gradient, 0)
        newgrad = [gradient, np.sum(gradient, 0)]
        return newgrad
...
class ReLU(FunctionNode):
    ...
    def forward(inputs):
        "*** YOUR CODE HERE ***"
        """I was looking through the numpy built in functions for the  ones 
        specified in the hint and found this funciton more fiting."""
        return np.clip(inputs[0], 0, None)

    @staticmethod
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        newImput = np.clip(inputs[0], 0, None)
        newImput = np.where(newImput > 0,1,0)
        val = newImput + gradient
        finalGrad = np.where(val > gradient, gradient, 0)
        #print finalGrad
        return [finalGrad]
